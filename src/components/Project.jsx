...
const Project = () => {
  ...
  return (
    <>
   {project.length > 0 && <div className="text-white py-4 pt-[150px] container flex flex-col items-center">
      ...
      {project.map((proj) => (
        <div className="w-full mb-8" key={proj._id}>
          <div className="w-full h-[380px] overflow-hidden rounded-t-[20px] relative md:h-[480px]">
            ...
            />
            <div className={`${showDetails === proj._id ? 'bg-primary text-white' : 'bg-white text-black'} absolute top-4 right-6 cursor-pointer flex justify-between items-center rounded-full py-2 px-5 transition-all duration-500 ease-in-out z-[999]`} onClick={()=>handleDetail(proj._id)}>
              <h1 className="uppercase">Details</h1>
              <span className="ml-1 text-xl">
              { showDetails === proj._id ? <GoDash />  : <GoPlus />}
              </span>
            </div>
            {showDetails === proj._id && <div className="flex flex-col justify-center w-full h-full top-0 right-0 bg-white rounded-lg md:top-[70px] md:right-6 text-black absolute md:h-[300px] md:w-[300px]">
              <p className="p-8 text-sm">
                <h2 className="font-bold uppercase text-[grey] mb-2 text-center">Project Details</h2>
                {proj.Detail}
                </p>
            </div>}
          </div>
          <div className="bg-black px-[15px] py-14 rounded-b-[20px] flex justify-between items-center relative md:px-[80px]">
            <span>
              ...
              <span className="py-2 text-sm inter px-5 rounded-full text-white bg-[#282828] uppercase mr-2">
              {stack.name}
            </span>
              ))}
            </span>
            <div className="live rounded-full h-[120px] w-[120px] flex justify-center items-center absolute right-5 top-[-30%] cursor-pointer md:relative">
              <img
                src={live}
